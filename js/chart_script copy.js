...
function addValueToChart(deviceName, batteryPercentage) {
    ...
    if (batteryChart) {
        ...
        console.log('timestamp ligne 113:', timestamp);
       ...
       getBatteryDataAndUpdateChart();
        /*createOrUpdateBatteryChart({
            ...
            labels: minutes.map(minute => {
                // Convertir chaque minute en format local d'affichage
                return new Date(minute).toLocaleString("fr-FR", { timeZone: "Europe/Paris" }).slice(11);
            }),
            datasets: datasets,
        });*/
    } else {
